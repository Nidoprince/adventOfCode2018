...
mostConsistantGuard = sorted(guardMinutesSlept,reverse=True,key=lambda x:guardMinutesSlept[x].count(max(set(guardMinutesSlept[x]),key=guardMinutesSlept[x].count,default=0)))[0]
sleepySchedule = guardMinutesSlept[mostConsistantGuard]
sleepiestMinute = max(set(sleepySchedule),key=sleepySchedule.count)
print(mostConsistantGuard,sleepiestMinute,mostConsistantGuard*sleepiestMinute)
